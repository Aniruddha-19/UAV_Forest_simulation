...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.SGD(params, lr=0.001, momentum=0.9, nesterov=True) #LR changed from 0.01 to 0.001
    # scheduler = StepLR(
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        writer.add_scalar('Loss/train', train_loss_hist.value, epoch + 1)
        writer.add_scalar('mAP/mAP@0.50', float(metric_summary['map_50']), epoch + 1)
        writer.add_scalar('mAP/mAP@0.50:0.95', float(metric_summary['map']), epoch + 1)
        writer.add_scalar('Learning_Rate', current_lr, epoch + 1)
        # Log per-class AP if available
        if 'map_per_class' in metric_summary and metric_summary['map_per_class'].dim() > 0:
            for cls_idx, ap_val in enumerate(metric_summary['map_per_class']):
                writer.add_scalar(f'AP_per_class/{CLASSES[cls_idx+1]}', float(ap_val), epoch + 1)
        ...
        scheduler.step(train_loss_hist.value)
